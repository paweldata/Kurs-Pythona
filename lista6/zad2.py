...
class NeuralNetwork:
    def __init__(self, x, y, f1, f1_derivative, f2, f2_derivative):
        self.input = x
    ...
    def __feedforward(self, i):
        ...
        self.output[i] = self.function2(np.dot(self.layer1, self.weights2.T))

    def __backprop(self, i):
        delta2 = (self.y[i] - self.output[i]) * self.function2_derivative(self.output[i])
...
def main():
    ...
    nn = NeuralNetwork(x, y, tanh, tanh_derivative, tanh, tanh_derivative)
    nn.showAnimation()
